...
fn format_cell(unit: &TrackUnit) -> String {
    ...
    // velocity: xmrs gives 0.0-1.0, show as 00-FF hex (like volume column)
    let vol = if unit.velocity > 0.0 && unit.velocity < 1.0 {
        #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
        let v = (unit.velocity * 255.0) as u8;
        format!("{:02X}", v)
    } else if unit.velocity >= 1.0 && unit.instrument.is_some() {
        // Full velocity with instrument = show it
        "FF".to_string()
    } else {
        "..".to_string()
    };
...
/// Format the first effect from a TrackUnit as a 4-char string.
///
/// XM effects are stored as structured types in xmrs, so we convert
/// back to a readable abbreviated form for display.
fn format_effects(unit: &TrackUnit) -> String {
    // Show first track effect, or first global effect, or "..."
    if let Some(effect) = unit.effects.first() {
        format_track_effect(effect)
    } else if let Some(effect) = unit.global_effects.first() {
        format_global_effect(effect)
    } else {
        "....".to_string()
    }
}
