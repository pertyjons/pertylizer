...
use synth_core::{BeatPosition, Bpm, Hertz, MidiNote, PortName, SampleCount, SampleRate};
use synth_core::{EnvelopeParam, ModuleType, OscillatorParam, Param};
use synth_engine::{ModuleGraph, ModuleId};
use synth_modules::{
    Amplifier, AudioBuffer, Envelope, InputPorts, Oscillator, PolyModule, ProcessContext,
    StereoOutput,
};
use synth_sequencer::{Duration, PatternTick, Pitch, SeqInstrumentId, Song, Tick, Velocity};
...
fn test_signal_probe_envelope_attack() {
    ...
    env.set_param(Param::Envelope(EnvelopeParam::Attack(
        synth_core::Seconds::new(0.01),
    )));

    // Trigger envelope
    env.note_on(MidiNote::C4, synth_core::Velocity::MAX);
...
fn test_sequencer_debugger_step_to_first_note() {
    ...
    // Verify it's C4
    if let synth_sequencer::SequencerEvent::NoteOn { pitch, .. } = &note_ons[0].event {
...
fn test_sequencer_debugger_step_through_pattern() {
    ...
    let pitches: Vec<u8> = note_ons
        .iter()
        .filter_map(|e| {
            if let synth_sequencer::SequencerEvent::NoteOn { pitch, .. } = &e.event {
                Some(pitch.as_midi())
            } else {
                None
            }
