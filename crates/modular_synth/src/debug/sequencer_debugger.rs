...
use synth_core::{Bpm, SampleRate};
use synth_engine::SequencerEngine;
use synth_sequencer::{Pitch, SeqInstrumentId, SequencerEvent, Song, Tick, TrackId};

/// Playback state (re-exported from sequencer_engine).
pub use synth_engine::PlayState;
...
impl SequencerDebugger {
    ...
    pub fn step_to(&mut self, target: Tick) -> Vec<TimestampedEvent> {
        ...
        while self.engine.current_tick() < target {
            ...
            self.engine.process(
                synth_core::SampleCount::new(samples_per_tick),
                &mut raw_events,
            );
    ...
    fn calculate_samples_per_tick(&self) -> usize {
        ...
        let ticks_per_second = beats_per_second * synth_sequencer::TICKS_PER_QUARTER as f32;
