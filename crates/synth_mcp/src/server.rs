...
impl SynthMcpServer {
    ...
    #[tool(
        description = "Install or clear a YAMS control script on a Mod Matrix (`mmx-N`) OR Script (`scr-N`) module slot (Step 2) — despite the name, `module_id` may be either. With a non-empty `source`, the script is compiled (a compile error is returned with diagnostics) and its `out` value becomes the slot's output: a Mod Matrix slot's modulation offset (replacing amount × source), or a Script module's `outN` port value. An empty `source` clears the slot back to scalar behaviour. `slot` is 1-based, matching get_mod_matrix_routings (Mod Matrix: 1..=16; Script module: 1..=8, driving out1..out8). YAMS reads `src`-bound module outputs/params (e.g. `src lfo = lfo-1.out`) plus macros (velocity, mod_wheel, …) and context (gate, age, cr, sr, note_hz), and assigns a normalized value to `out`. Read installed scripts back via get_mod_matrix_routings (mmx) or get_module_info (scr `scripts` array); see get_yams_reference for the language."
    )]
