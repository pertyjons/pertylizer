...
//! - `descriptors.json` — machine-readable module, port, compatibility, and
//!   parameter metadata for every module type, so a headless file exporter can
//!   validate graphs and replicate the engine's normalization
...
//! schema can be used to pick a format. The nested `Song` is fully derived
//! via `JsonSchema`.
...
/// Emit `descriptors.json`: the offline module-capability catalog.
///
...
/// discriminator and parameter keys used in saved `.ptz` files.
fn write_descriptors(out_dir: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &mt in ALL_MODULE_TYPES.iter() {
        ...
        let ports: Vec<Value> = desc.ports.iter().map(port_descriptor).collect();
        modules.insert(
    ...
    let doc = json!({
        "$comment": "Machine-readable capability descriptors for every module type. \
            NOTE: this is NOT a JSON Schema — it is a bespoke parameter-capability catalog \
            (a discovery manifest for the app and MCP/AI clients), describing each module's \
            classification, ports, compatible connections, and parameters. The real JSON \
            Schemas live alongside it: \
            project.schema.json / patch.schema.json / \
            bundle-metadata.schema.json (generated by schemars) validate file structure; this \
            file instead carries discovery semantics JSON Schema has no vocabulary for. Port \
            `direction`, `signal_type`, and `value_domain.id` ids are lowercase. A port's \
            `value_domain` distinguishes exact accepted values from its optional nominal range \
            and unit; the nominal range is descriptive and not an engine clamp. \
            `connection_compatibility` maps each output signal type to the input signal types it \
            can drive. Parameter `type` borrows JSON Schema's primitive names \
            (number/integer/boolean) only as a hint for which JSON primitive to send. \
            Generated by `cargo run -p pertylizer --bin gen_schemas` — do not edit by hand. \
            Keys match the `type` discriminator and parameter keys in saved `.ptz` files. \
...
/// Build one stable, lowercase port catalog entry from its source descriptor.
fn port_descriptor(port: &PortDescriptor) -> Value {
    json!({
        "name": port.name,
        "label": port.label,
        "description": port.description,
        "direction": port.direction.id(),
        "signal_type": port.port_type.id(),
        "value_domain": port_value_domain(port.value_domain),
    })
}

/// Build the accepted-value contract for one port.
fn port_value_domain(domain: synth_core::PortValueDomain) -> Value {
    let mut value = serde_json::Map::new();
    value.insert("id".to_owned(), json!(domain.id()));
    value.insert(
        "accepted_values".to_owned(),
        json!(domain.accepted_values()),
    );
    if let Some(range) = domain.nominal_range() {
        value.insert("nominal_min".to_owned(), json!(range.min()));
        value.insert("nominal_max".to_owned(), json!(range.max()));
    }
    if let Some(unit) = domain.unit() {
        value.insert("unit".to_owned(), json!(unit));
    }
    Value::Object(value)
}

/// Map each output signal type to the input signal types it may drive.
///
/// This is derived from [`PortType::can_drive`] so the offline catalog cannot
/// drift from graph, GUI, or MCP validation.
fn connection_compatibility() -> Value {
    let mut compatibility = serde_json::Map::new();
    for source in PortType::ALL {
        let destinations: Vec<&str> = PortType::ALL
            .into_iter()
            .filter(|destination| source.can_drive(*destination))
            .map(PortType::id)
            .collect();
        compatibility.insert(source.id().to_owned(), json!(destinations));
    }
