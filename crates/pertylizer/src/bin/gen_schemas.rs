...
fn write_descriptors(out_dir: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let doc = json!({
        "$comment": "Machine-readable capability descriptors for every module type. \
            ...
            `direction` and `signal_type` ids are lowercase; `connection_compatibility` maps \
            each output signal type to the input signal types it can drive. Parameter `type` \
            borrows JSON Schema's primitive names (number/integer/boolean) only as a hint for \
            which JSON primitive to send. \
