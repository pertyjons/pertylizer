...
mod tests {
    ...
    /// `set_voice_pitch` retunes the grain trigger rate (F0) via `note_freq`
    /// (read live by `process`): 2× voice pitch doubles the rendered
    /// fundamental; a static note holds. Measured at A2 (formant-rich grains).
    #[test]
