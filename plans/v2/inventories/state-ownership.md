...
**All 64 entries are `Classified`** as of 2026-08-29, so the register is `Current` against the revision its last audit pass names.
...
| STATE-0002 | `version` | `String`, `ProjectFile::FORMAT_VERSION` = `"1.1"` | `ProjectFile` | Every save path | Dirty: never — a constant. Undo: N/A | Project document | Replaced by the Format V2 envelope's own version, with STATE-0001. A V1 `"1.1"` document must be refused or converted, never reinterpreted under the new meaning; the envelope and its conversion policy are P00B-T007 and the unknown-field half is ADR-0016 | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0005 | `instruments[]` | `Vec<InstrumentState>` | `ProjectFile` | Engine snapshots via `build_project_from_engine`, overlaid with GUI `PatchEditor` metadata | Dirty: `graph`. Undo: `SetRackModules` (a whole-rack snapshot with severed connections and a direction flag); **no dedicated instrument add/remove action** | Project document | The list moves as authored content, and its order may keep the meaning it has — `active_instrument_id` falls back to the first element (STATE-0004). What must change is that **nothing identifies an instrument by its position**: `SequencerTrack.instrument` and `AutomationTarget::Instrument` address one by id today, and the master plan forbids display order as identity. The identity itself is `IDN-0001` under ADR-0014 | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0006 | `song` | `Song` | `SharedSong` (`RwLock` + `ArcSwap`) | Read from `SharedSong` at save | Dirty: `song`. Undo: broad — see the song rows below | Project document | The section moves, but the V2 `song` is **not** the V1 `Song` struct: two of its current members are re-homed by this ledger — the transport loop stays only by decision (STATE-0045) and the allocator cursors become one record (STATE-0046) | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0010 | `global.return_bus_effects[]` | `Vec<ReturnBusEffectsState>` | Engine return buses | Reconstructed from engine at save. The bus *identity and fader* live in `song.return_busses`; only the effect chain lives here — one concept split across two sections of the file. | Dirty: `global` fingerprint (it hangs off `EngineState`, so the `graph` counter never saw it — this term had to be added after the fact). Undo: `SetChainEffect`, `SetChainEffectParameter`, `SetChainEffectBypass` | Project document | The migration **is** the rejoin. A return bus is one concept stored in two file sections, and V2 must give the bus a single owner carrying identity, fader and chain together, rather than reproducing the split that made this chain invisible to the `graph` counter | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0011 | `global.master_effects[]` | `Vec<ModuleState>`, chain order = array order | Engine master bus | Reconstructed from engine at save | Dirty: `global` fingerprint — adding a master effect is the documented case that reported clean before this term existed. Undo: same three chain actions | Project document | Array position expresses **processing order**, not identity, and `IDN-0021` records the open question these chains actually carry: they live in `global`, outside the patch that owns every other module, so their module ids are allocated in a different namespace whose overlap with a patch's is unchecked. The per-patch chain expresses the same order differently, as `IDN-0020` and STATE-0033 | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |

### Project document — per instrument

| ID | Field/state | Domain type | Current owner | Mirrors/save sources | Dirty/undo behavior | Intended V2 owner | Migration | Evidence | Status |
|----|-------------|-------------|---------------|----------------------|---------------------|-------------------|-----------|----------|--------|
| STATE-0012 | `instruments[].id` | `InstrumentId` | Engine | Engine snapshot | Dirty: `graph`. Undo: N/A — identity, not a value | Project document | This identity is `IDN-0001`, and its generation and encoding is [ADR-0014](../decisions/ADR-0014-persistent-id-generation-and-encoding.md): a bare integer becomes an origin plus a never-reused ordinal. `IDN-0029`'s audible-renumbering edge belongs to the **module** id, not this one — `script_seed_base` folds the voice index and the module type and instance, and no instrument id | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0013 | `instruments[].name`, `.description`, `.color` | `String`, `String`, `Option<String>` | Engine/GUI | Engine snapshot | Dirty: `ui` (instrument properties). Undo: **none** | Project document | `color` is an `Option<String>` hex string while a track, return bus or graph uses `TrackColor { r, g, b }` and a section uses `SectionColor { red, green, blue }` — **three encodings of one concept** in one document. V2 needs one, and converting the other two is the migration | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0015 | `instruments[].channel` | MIDI channel selection | Engine | Engine snapshot | Dirty: `ui`. Undo: `SetInstrumentSettings` | Project document | The persisted form is an integer where **`0` means OMNI** (`project_flow.rs:704`), a sentinel inside the value range. `MidiChannelSelection` already exists as the domain type; V2 should persist that rather than the sentinel | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0016 | `instruments[].key_range` | array | Engine | Engine snapshot; also settable by MIDI learn (`EngineEvent::KeyRangeLearned`) | Dirty: `ui`. Undo: `SetInstrumentSettings`. Whether the MIDI-learn path pushes an undo entry is **not** established | Project document | A positional two-element array. V2 should carry a named low/high range so the pair cannot be read in the wrong order. Whether the MIDI-learn path (`EngineEvent::KeyRangeLearned`) pushes an undo entry is still **not established** | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0020 | `instruments[].volume`, `.pan` | `NormalizedValue`, `BipolarValue` | Engine mixer | Engine snapshot | Dirty: `ui`. Undo: `SetInstrumentSettings` | Project document | Instrument volume and pan are one of **two gain stages** for the same audio — the track carries its own (STATE-0039). Which survives, and whether both do, is ADR-0034's track/source/channel ownership question | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0022 | `instruments[].velocity_amp_sensitivity`, `.velocity_filter_sensitivity` | numbers | Engine | Engine snapshot | Dirty: `ui`. Undo: `SetInstrumentSettings` | Project document | `N/A` — two scalar sensitivities with no identity, no mirror and no shape question. They move as authored instrument data | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0025 | `patch.modules[]` | `Vec<ModuleState>` (75-variant enum) | Engine graph, mirrored in `shared_graph` | Engine reconstruction; `shared_graph` mirrors commands **by id**, so it is not a valid oracle for engine state | Dirty: `graph`. Undo: `SetRackModules` | Project document | The 75-variant `ModuleState` enum is what Phase 5's declarative node API replaces, together with `SOUND-INV-012`'s uncovered second sentence. Its `shared_graph` mirror is **not** a valid oracle for engine state, so a V2 reconstruction must not be written against it | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0027 | `module.position` | `Position { x, y }` | **GUI `PatchEditor` only** | Present in the project **only** via the GUI overlay. `build_project_from_engine` writes `Position::default()`. | Dirty: `layout` fingerprint — derived from the data precisely because the canvas has ~30 mutation points and none reported themselves. Undo: **none for a drag**; a position only rides along inside a `SetRackModules` snapshot | Project document (`EditorMetadata`) | Reachable **only** through the GUI overlay: `build_project_from_engine` writes `Position::default()`, so the MCP timeout fallback silently saves a project with every module at the origin. V2 must give `EditorMetadata` one writer instead of an overlay that a timeout can skip | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0028 | `module.description` | `String` | GUI / MCP `set_module_description` | Engine snapshot | Dirty: `graph`, established 2026-08-29. Neither the GUI nor MCP calls `SharedGraph::set_module_description` directly, but `CommandSender::send` calls `control_snapshot::publish` before enqueueing (`synth_engine.rs:204`), and its `SetModuleDescription` arm calls it (`control_snapshot.rs:75`), bumping the version the `graph` counter reads. That command names an instrument, so it is the **instrument-module** description this establishes; a master or return-chain description does not travel it. Undo: **none** | Project document | This entry spans three places a module can sit — a patch, the master chain and a return chain — and only the patch case is traced. An instrument-module description is carried by the same control-snapshot publication that makes it dirty, so V2 must keep the two together: a metadata field routed around `control_snapshot::publish` would be both unobserved and unsaved. The master and return chains take a different path and are **not** established here, nor is whether any of the three survives an actual save and reload — the existing round-trip test exercises set, read and clear through the bridge, not a project write. Both are owed to P00B-T005 | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0031 | `patch.groups[]` | `Vec<ModuleGroupState>` — name, color, position, collapsed, members, exposed ports | **GUI `PatchEditor` only** | GUI overlay only; lost on the engine fallback path | Dirty: `layout` fingerprint. Undo: **none** | Project document (`EditorMetadata`) | Same overlay-loss path as STATE-0027, and worse: groups exist **only** in the GUI copy, so the engine fallback writes none at all. See also STATE-0055, where a user's saved templates carry the same shapes outside the project | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0032 | `patch.settings.canvas_size` | `Option<CanvasSize>` | **GUI `PatchEditor` only** | GUI overlay only | Dirty: `layout` fingerprint. Undo: **none** | Project document (`EditorMetadata`) | Same overlay-loss path as STATE-0027. It is presentation intent with no audible effect, which is what makes ADR-0018's shared-content question rather than ADR-0013's | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0037 | `song.default_tempo`, `.default_time_signature`, `.row_resolution` | `Bpm`, `TimeSignature`, `RowResolution` | `SharedSong` | GUI, MCP | Dirty: `song`. Undo: `SetTempo`, `SetTimeSignature`; `row_resolution` has **none** | Project document | Two unrelated questions share this row. The tempo and time-signature defaults are the **timeline segment before the first applicable change** in STATE-0038's lists, so they are complementary to the maps and may legitimately differ from every entry in them; V2 must keep that baseline explicit rather than deriving it. `row_resolution` is not part of either map — it is the piano-roll grid division — and it has no undo action, so an edit to it cannot be undone even though it changes how a pattern is read | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0038 | `song.tempo_changes[]`, `.time_signature_changes[]` | tempo map | `SharedSong` | GUI, MCP `set_tempo_at` | Dirty: `song`. Undo: `SetTempo`, `MoveTempo`, `SetTimeSignature` | Project document | A tempo change carries a `ramp` flag, and the law that flag selects is [ADR-0049](../ADR.md), registered for Phase 3/4 and not yet decided. The map must migrate under whichever law is accepted, not before | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0040 | `song.patterns[]` | `Vec<Pattern>` — notes, automation, processors, note graph. Its `next_note_id` cursor is **not** here: an allocator cursor is STATE-0046, so it appears exactly once | `SharedSong` | GUI, MCP | Dirty: `song`. Undo: the largest group — 17 note actions, 6 automation actions, `AddPattern`, `DeletePattern`, `RenamePattern`, `SwapPattern`, `SetPatternLength`, `FreezePattern`, `RestorePattern`, `SetPatternNoteGraph`. **`IDN-0027`: undoing a note deletion allocates a new `NoteId`** | Project document | The largest structure here, and the one ADR-0014 changes most: note identity becomes document-scoped, so `IDN-0004`'s "a `NoteId` must always be paired with a `PatternId`" goes away. `next_note_id` is **not** part of this entry — see STATE-0046 | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0041 | `song.arrangement[]` | `Vec<PatternPlacement>` | `SharedSong` | GUI, MCP `place_pattern` | Dirty: `song`. Undo: `InsertPlacement`, `RemovePlacement`, `MovePlacement`, `SetPlacementLength`, `SetPlacementLoopMode` | Project document | Every placement is a **cross-boundary reference**, to a pattern and a track it does not own. ADR-0014's never-reused ordinal is what keeps a placement from silently addressing whatever later occupies a freed id | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0042 | `song.sections[]` | `Vec<ArrangementSection>` — id, name, kind, color, start, length | `SharedSong` | GUI | Dirty: `song`. Undo: `SetArrangementSections` (whole-list snapshot) | Project document | Sections use a third color encoding, `SectionColor { red, green, blue }` — see STATE-0013. Their `start` and `length` are positions in the same arrangement space as STATE-0041, but a section holds no reference to the placements it spans — `Song` keeps the list sorted by `start` and nothing else — so moving a placement neither moves nor invalidates the section around it | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0049 | `AppSettings.author` | `Author` | `settings.json` | **Seed only.** Cloned into `current_project_author` at startup (`egui_backend.rs:608`) and used as the fallback when a loaded project carries no author. It is *not* the source of `ProjectFile.author` at save — see STATE-0003, which the first two passes both got wrong. | Dirty: never — editing it does not change the open project | User settings | Its **seed-only** role must survive the collapse STATE-0003 and STATE-0060 require. Folding all three author holders into one owner must not fold this one in: it is a user preference for new projects, not a copy of the open project's author | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0050 | `AppSettings.directories` | 5 optional paths (patches, projects, last open/save/project dir) | `settings.json` | GUI file dialogs | Dirty: never. Undo: N/A | User settings | `N/A` — five per-user paths with no meaning inside a project and no reference into one | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0051 | `AppSettings.window` | width, height, x, y | `settings.json` | GUI | Dirty: never. Undo: N/A | User settings | `N/A` — window geometry, per user and per machine. It has no project meaning and no V2 question beyond staying out of the document | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0052 | `AppSettings.recent_projects` | `Vec<PathBuf>`, max 10 (`LIMIT-0054`) | `settings.json` | GUI | Dirty: never. Undo: N/A | User settings | `N/A` as data, but note the shape: a bare `Vec<PathBuf>` capped at 10 (`LIMIT-0054`), so a moved or deleted project leaves a stale entry with nothing to relink against. That is a UI concern, not a document one | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
| STATE-0053 | `AppSettings.load_warning` | `Option<String>`, `#[serde(skip)]` | In-memory only | Set when settings fail to load | Dirty: never. Undo: N/A | Frontend-local transient | `N/A` — `#[serde(skip)]`, set only when settings fail to load and never written anywhere | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| STATE-0055 | User group templates | `GroupTemplate` JSON files | `GroupTemplateManager` | Read from disk alongside 12 built-ins | Dirty: never — not project state. Undo: N/A | User settings | A user's templates embed `Vec<ModuleState>`, `Vec<ConnectionState>` and `Vec<ExposedPortState>` — the **same shapes as STATE-0025, STATE-0030 and STATE-0031**. They therefore pin the project's module and group format from outside the project, and no project-format migration reaches them: changing those shapes silently breaks every template a user has saved | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) | Classified |
...
| 2026-08-29 | `278c7c26` | `schemas/project.schema.json` walked mechanically; `synth_engine/src/shared_state.rs:579`, `pertylizer/src/session.rs:390-407`, `gui/egui_backend.rs:3204-3212`, `mcp_bridge/instruments.rs:715-729`, `crates/pertylizer/tests/mcp_project_load.rs:465`; `synth_sequencer/src/song.rs:506-556`; the schema's three colour definitions and `InstrumentState.key_range`. | The 27 blank `Migration` cells filled, so all 64 entries are now `Classified` under the row-level status rule the coverage check enforces. Four findings worth the phase's attention. **A patch module's description is `graph`-dirty after all** (STATE-0028), which this pass first recorded as observed by no term at all: the trace stopped at the two direct callers and missed `control_snapshot::publish`, which `EngineCommandSender::send` runs before enqueueing and whose `SetModuleDescription` arm bumps the version. An independent review caught it. Then: **one concept has three colour encodings** in one document (STATE-0013); **a user's group templates embed the project's own module, connection and exposed-port shapes** from outside the project, where no format migration reaches them (STATE-0055); and a section holds no reference to the placements it spans (STATE-0042). Method limit: every path here was traced by reading, not executed, and the module-description trace shows what that costs — a caller reached through a generic publication step is invisible to a search for direct callers. Whether the description survives an actual save and reload is left to P00B-T005. | [EVD-0018](../evidence/phase-00b/EVD-0018-state-ownership-coverage.md) |
