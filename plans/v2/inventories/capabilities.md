...
| CAP-0017 | Engine  | Multi-client hub (`hub.rs`) — per-client event buffers, `ClientId`                                                                                                               | Exported; shipped reachability not established |             |                      | Named by the master plan as a possibly tested-only subsystem | Investigating |
...
| CAP-0047 | Build      | Cargo features — `gui-egui`, `mcp`, `osc` (default), `rt-profiling`, `egui-inspection` (opt-in); MSRV 1.97; CI checks `--no-default-features` and `--all-features`                 | Build matrix                 |             |                      | Supported matrix is ADR-0031                                                             | Discovered    |
...
| 2026-08-12 | `dd69b657`      | `AppShortcut::ALL` read from `gui/shortcuts.rs` (a closed 7-element table that also renders the menu); MCP annotation attributes counted by `rg -o` over `read_only_hint`/`destructive_hint`/`idempotent_hint`; `dialog_flow.rs` read for the startup ordering.                                                                                                                                                                                                                                                                                                                                                                                   | 8 entries added (`CAP-0048`..`CAP-0055`); `CAP-0011` upgraded to `Classified` once the read/mutate split was separated — annotation coverage turned out to be complete. Gaps that remain and are deliberate: **no entry has a disposition**, because assigning one is decision work that the master plan places after ADR-0030 and ADR-0031, and per-item rows for the 219 tools, 76 commands, 75 module types, and 68 patches are mechanical expansion best done once the disposition rules exist, so they do not repeat 438 guesses. `CAP-0017` (multi-client hub) remains the one surface whose *shipped* reachability is unknown. | Pending `EVD` record for P00B-T002 |
