...
Next action: **merge `P06-S001`** on its one independent read, then draft the allocation and
stealing ADR before `P06-S002`. `SOUND-INV-025` states the instance model the slice built; the
ADR decides which voice a note-on takes when every index is held, which the slice refuses.
