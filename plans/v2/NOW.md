...
| P06-S002 — voice stealing under a decided policy | **Blocked** on ADR-0058, `Proposed` 2026-09-05 and put to the user | Needs the allocation and stealing ADR first: which voice is taken (V1 offers oldest, quietest, lowest priority, same note, none), how the taken voice ends at a precise sample (a release-then-retrigger over the retirement crossfade, or a cut), and what a stolen voice's expression keeps or clears — one documented rule, tested at sample offsets |
...
Two streams are active: Phase 6, with `P06-S001` merged and `P06-S002` next, and Phase 0B,
with `P00B-T003` as its selected slice.

Next action: **the user decides ADR-0058** — which voice a note-on takes when every index is
held, how it ends, and what its identity becomes; the record proposes `None` (today's
behaviour, the default), `Oldest` and `SameNote` over V1's fade-then-start. `P06-S002` builds
the accepted option; until then a full producer refuses and counts, as `SOUND-INV-025` states.
