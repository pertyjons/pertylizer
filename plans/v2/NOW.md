...
| P06-S002 — voice stealing under a decided policy | **Built** 2026-09-05 on `feat/v2-phase6-s002`, awaiting its independent read; ADR-0058 accepted with it | Needs the allocation and stealing ADR first: which voice is taken (V1 offers oldest, quietest, lowest priority, same note, none), how the taken voice ends at a precise sample (a release-then-retrigger over the retirement crossfade, or a cut), and what a stolen voice's expression keeps or clears — one documented rule, tested at sample offsets |
...
Next action: **merge `P06-S002`** on its one independent read, which also reviews ADR-0058;
then `P06-S002b`, the live boundary's steal, or `P06-S003`.
