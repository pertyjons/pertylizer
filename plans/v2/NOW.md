...
| P05-S007 — the parameter slot: laws, layers and the ramp segment | **Selected** 2026-09-04 | Build `SOUND-INV-023` and `SOUND-INV-024`, accepted the same day as ADR-0007 and ADR-0006: a `ParamSpec` law per addressable parameter, a parameter slot that composes the layers in the master plan's order and holds a linear segment, a kernel that reads one resolved value per sample and composes nothing, and `node::catalog()` presenting the law. **Completion check:** the two invariants' conformance rows filled — per law a resolved value against the record's arithmetic; an override leaving modulation in force; a not-modulatable write refused; a mid-segment retarget continuing from the current value; a gate and a pitch destination unsmoothed at their render position; the loop's advance one add per frame under the purity scan — mutation-verified, with renders bit-identical where nothing composes (every existing plan writes steps to a base, which the slot must reproduce exactly). Real-time boundary: the slot advances in the loop, so the core Rust gate and one independent review apply. Slice 8 — latency, tail, reset, cost and tap capability in the declaration — follows |
...
Two streams are active: Phase 5, with `P05-S007` selected, and Phase 0B, with `P00B-T003`
as its selected slice.

Next action: **build `P05-S007`** on a branch off `main`. ADR-0006 and ADR-0007 are accepted, so
the slot can be composed under a decided law and a decided ramp; its completion check is in the
table above.
