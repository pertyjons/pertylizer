...
Two streams are active: Phase 5, with `P05-S008` built and under review, and
Phase 0B, with `P00B-T003` as its selected slice.

Next action: merge `P05-S008` on its review, then open the Phase 5 exit review (`REV-P05`)
against the master plan's six exit bullets.
