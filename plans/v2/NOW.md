...
Next action: **draft the allocation and stealing ADR**, the decision `P06-S002` waits on.
`SOUND-INV-025` states the instance model `P06-S001` built; the ADR decides which voice a
note-on takes when every index is held, which the slice refuses and counts today.
