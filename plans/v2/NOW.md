...
Two streams are active: Phase 5, with `P05-S007b` merged and `P05-S008` next to select, and
Phase 0B, with `P00B-T003` as its selected slice.

Next action: select `P05-S008` — latency, tail, reset, resource cost and tap capability in
the declaration — against the phase's exit gate, then build it on a branch off `main`.
