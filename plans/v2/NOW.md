...
|---|---|---|
| P05-S001 — one declaration for one kind | **Built** 2026-09-04, awaiting its gate and review | `synth_engine_v2` now has a `NodeDeclaration` and the `Saw` kind is the first declared one: `descriptor`, `prepared_payload_bytes` and `state_payload_bytes` derive their `Saw` facts from it, and the arm each keeps — the registry matches exhaustively, so a declared kind still appears in each function — defers to the declaration and states nothing. `prepare` still reads the kind's IR fields until parameters become slots. **Completion check, as built:** (1) `SOUND-INV-012` has its conformance row — the region was already kind-blind under `the_render_loop_makes_no_topology_or_naming_decision`, so the new guard adds what that scan did not hold: one `Kernel::run` site and no kernel called by name in the hot path, mutation-verified both ways; (2) `a_declared_kinds_registry_facts_derive_from_its_declaration` reads every registry fact back and finds the declaration's, and `a_declared_kind_appears_in_the_registry_only_by_deferring_to_its_declaration` holds every `Saw` arm to an exact one-line forwarding form — a drifting value, a restated fact, a forwarding of nothing and a body on the next line are each caught, as are a second dispatch line, a kernel called by name and a kernel constant invoked in the hot path; (3) the sawtooth renders bit-identically: EVD-0013's `v2-aligned.wav` hashes as before and EVD-0012's digests reproduce. **Out of scope, by decision:** `ParamSpec` modulation laws, central parameter slots, the legacy adapter, discovery surfaces and a second kind — each a later slice; a field no consumer reads is not built here. The purity-scanned region gained a test and no code, and `node.rs` runs at admission only; the core Rust gate and one independent review still apply because the slice changes the registry the real-time path is resolved from |
...
Next action: **gate, review and commit `P05-S001`** on `feat/v2-phase5-s001`, then squash-merge
it to `main` under the merge rule; its completion check is in the table above and is what the
commit and review are judged against. After the merge, select slice 2.
