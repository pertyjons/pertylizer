...
Next action: **merge `P06-S002b`** on its one independent read; then `P06-S003`. The row's
first design — a renderer-side deferral for both paths — was not built: the deferral lives in
the ingress store, published by the drain, and the compiled path keeps its stamped events, so
the renderer stayed unchanged and ADR-0058 clause 7 stands as corrected.
