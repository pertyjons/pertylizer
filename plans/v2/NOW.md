...
Two streams are active: Phase 5, with slices 1–8 merged and the exit review next, and
Phase 0B, with `P00B-T003` as its selected slice.

Next action: open the Phase 5 exit review (`REV-P05`) against the master plan's six exit
bullets. Two decisions are the user's before or at that review: whether an oscillator's
amplitude declares `Smoothing::Quantum` (V1 de-zippers its level per block; the
render-contract tests pin a step), and whether the `LegacyPolyModuleAdapter` measurement
row is discharged or dropped, since no adapter exists and the renderer needs none.
