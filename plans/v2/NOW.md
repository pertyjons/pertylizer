...
| P05-S007b — the parameter slot: the ramp segment | **Built** 2026-09-04 on `feat/v2-phase5-s007b`, under review | `SOUND-INV-024` built: `Smoothing` per `ControlSpec` (every declared policy `None`, renders bit-identical), the segment in `SlotState` advanced once per quantum into a per-slot control buffer before the schedule walk, the sine and sawtooth reading their amplitude per frame from it, node state without a quantum-rate control, adoption seeding every slot so a catch-up never ramps, the buffers charged to `mutable_state_bytes`. Open: whether the amplitude de-zippers over a quantum as V1's level does — a delivered-behaviour decision for the user. Slice 8 — latency, tail, reset, cost and tap capability in the declaration — follows |
...
Two streams are active: Phase 5, with `P05-S007b` built and under review, and
Phase 0B, with `P00B-T003` as its selected slice.

Next action: merge `P05-S007b` on its review, then select `P05-S008` — latency, tail, reset,
resource cost and tap capability in the declaration — and build it on a branch off `main`.
