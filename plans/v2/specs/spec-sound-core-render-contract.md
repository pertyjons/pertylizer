...
24. **SOUND-INV-024 — A parameter ramp is a linear segment in the slot.**
    ...
    **Not built.** Every write is a step today: `P05-S007a` built the slot the segment lives
    in and every write reaches a kernel through it, so `P05-S007b` adds the segment to that
    slot and the per-frame read to the loop, under this invariant.
...
| SOUND-INV-024 | **Not built.** Owed by `P05-S007b`, into the slot `P05-S007a` built: a segment's first frame reading past the start value and its last frame reading exactly the target; a retarget mid-segment continuing from the current value; a gate and a pitch destination landing exactly at their render position with no smoothing; an activation's catch-up seeding a slot with no frames remaining; the loop's advance being one add per frame under the purity scan |
