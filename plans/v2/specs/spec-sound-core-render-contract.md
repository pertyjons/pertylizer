...
25. **SOUND-INV-025 — A voice scope is one prepared shape and `N` instances of state.**
    ...
    **Built by `P06-S001` and `P06-S002`.** Instances are `Lowering::schedule` over one
    `PreparedSlot`, the voice sum is the `copy` and `accumulate` kernels, rows are
    ...
    groups are the plan's `instance_groups` and `sum_groups`. The live boundary's steal —
    ADR-0058 clause 6's second site — is `P06-S002b`'s and is owed. The conformance row lists
    the falsifiers.
