...
25. **SOUND-INV-025 — A voice scope is one prepared shape and `N` instances of state.**
    ...
    **Built by `P06-S001`.** Instances are `Lowering::schedule` over one `PreparedSlot`, the
    voice sum is the `copy` and `accumulate` kernels, rows are `ParameterTarget::instances`,
    routing is `voice_row` in `render/hot.rs`, the catch-up is one row per address, and the
    stealing decision is `P06-S002`'s. Its conformance row lists the falsifiers.
