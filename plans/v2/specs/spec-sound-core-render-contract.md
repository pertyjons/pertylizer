...
22. **SOUND-INV-022 — An observation tap is a declared, passive compiler artifact.**
    ...
    **Not built.** No node kind declares a tap and no plan carries one. Phase 5's first
    observation slice builds it under this invariant, which binds that slice rather than
...
23. **SOUND-INV-023 — One modulation law per parameter, composed in the slot.**
    ...
    **Not built.** No slot composes and no declaration names a law. Phase 5's seventh slice
    builds it under this invariant, which binds that slice rather than describing current
    code; `node::catalog()` presents the law beside the unit from then on.
...
24. **SOUND-INV-024 — A parameter ramp is a linear segment in the slot.**
    ...
    **Not built.** Every write is a step today. Phase 5's seventh slice builds the segment
    with the slot, under this invariant.
...
| SOUND-INV-022 | **Not built.** Owed by Phase 5's first observation slice, which brings the equivalence test the master plan's Phase 5 exit gate names — observation disabled, enabled and saturated render bit-identical samples and one semantic digest — and a test that a plan compiled with no subscriber holds the same declared taps as one compiled with every observation enabled |
| SOUND-INV-023 | **Not built.** Owed by Phase 5's seventh slice: per law, a resolved value against the record's arithmetic; an automation override that leaves a modulation in force; a write to a not-modulatable parameter refused at admission; a catch-up write taken as the override layer; and the purity scan finding no composition in `node/kernels.rs` |
| SOUND-INV-024 | **Not built.** Owed by the same slice: a segment's first frame reading past the start value and its last frame reading exactly the target; a retarget mid-segment continuing from the current value; a gate and a pitch destination landing exactly at their render position with no smoothing; an activation's catch-up seeding a slot with no frames remaining; the loop's advance being one add per frame under the purity scan |
