...
| Six producer-share values and `release_hold_capacity` | Measure useful fixed profile inputs, validate every construction relation and checked sum, and reselect `max_events_per_quantum` from those measurements before enabling live ingress | The unevidenced value 256 is not retained merely because the measured partition fits. Construction never derives shares from a plan; every share and `release_hold_capacity` is a positive `EventCount`, a disabled optional class leaves its share unused, and `release_event_share >= release_hold_capacity` |
