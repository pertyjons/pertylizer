...
9. **HOST-INV-009** — Within live input, **dropping** at runtime is permitted only for queues explicitly marked *Live
   ...
   shortage that can cause one, and the remaining causes are licensed here rather than left
   to HOST-INV-021 to invent: a live note-on that cannot atomically acquire its queue slot, its producer
   release hold **and** an identity from its producer's range is dropped at that same live boundary, before
   acceptance, and never later. It is counted against the
   queue it was offered to, with the exhausted resource — slot, hold or identity — named, so the three causes stay
   distinguishable
   in the report. This remains the one live-input drop behaviour: no other live-input shortage may be discharged as a
   drop.

   **Two note-edge causes are counted here and are not drops, and the difference is not bookkeeping.** A drop loses an
   event the producer was entitled to send; these two refuse an event that names no live note. An edge carrying an
   identity that names no live note — a free index, a superseded generation, or a retired
   index — is an **orphan**, and one whose producer holds no identity because its note-on was dropped is
   **never-minted**. Both are refused, counted against the producer that
   offered them and reported — but neither consumes the drop licence above, and reporting an orphan as a drop would
   make a producer look starved when it is in fact releasing a note that does not exist.
   Engine-egress dropping is outside this registry and is separately licensed by ADR-0038 part 1. This invariant
...
13. **HOST-INV-013** — `forward_event_horizon >= maximum_block_size + Q`, and it binds only events whose provenance is
    `Hardware`, `Arrival` or `Simulated` (ADR-0032 clause 21, as ADR-0053 amends it alongside clause 18). It never
    measures the scheduler's own releases of compiled events, and there is no backward horizon.

    **`Simulated` is ingress for this rule and exact for every other.** ADR-0053 gives a deterministic in-engine
    producer its own provenance because none of the other three fits: its timestamp is exact by construction, like
    `Compiled`'s, while its origin is outside the renderer, like the other two. Exempting it — which naming it
    `Compiled` would do — is what would make the equivalence fixture that proves this boundary unable to reach it. It
...
21. **HOST-INV-021 — destination admission makes renderer capacity a construction invariant.**
    ...
    **Six disjoint profile shares sit within the total.** Every renderer event is charged exactly once, to compiled
    ...
    - `max_scheduled_events_in_flight` to cover `compiled_event_share` times
      `max_quanta_per_callback = ceil(maximum_block_size / Q)`; and
    ...
    - **the note-identity index space is at least `max_held_notes`** (ADR-0047 clause 5). An index addresses one
      simultaneously outstanding obligation and `max_held_notes` bounds how many exist, so a narrower index space
      would let a profile name more held notes than it can identify. `max_held_notes` is constrained only to be
      nonzero on its own, so this relation is what closes that — a construction refusal rather than an assumption
      about which profiles are reasonable.

    One further relation binds the same partition but **not the profile**, and separating them is what makes each one
    checkable at the place that can decide it. **The sealed-batch store covers
    `max_events_per_quantum * max_quanta_per_callback`.** That store is the publication arbiter's, not a profile
    field, so the profile cannot check its size — what construction does check is that the extent is nameable as an
    `EventCount` at all, and the arbiter's preparation allocates exactly it.
    ...
    **Live input may lose only a new event at its bounded boundary.** Queue overflow, or a note-on that cannot
    atomically acquire its event slot, its live-producer hold **and** an identity from its producer's range, is
    dropped before renderer publication, counted and reported under HOST-INV-009 with the exhausted resource named.
    An edge refused because it names no live note is **not** a drop: HOST-INV-009's orphan and never-minted cases are
    refusals, counted separately, and folding them into this loss would make a healthy producer look starved. An entry already accepted into the eligible snapshot waits only for its destination to enter the
    ...
    individual release redeems one hold into the release share.

    **A hold is acquired against a note identity and redeemed by one.** ADR-0047 supplies it: a note-on mints an
    identity from its producer's disjoint range in the same indivisible step that takes the queue slot and the hold,
    and a release names that identity to redeem it. Without one, "the matching release" is not something the renderer
    can recognise — a release carrying only a node address is indistinguishable from an orphan, which is the case
    HOST-INV-009 above now counts and which this clause's guarantee depends on being able to tell apart. A mass
    release redeems every hold in its scope by advancing the affected identities, which is why it emits no per-voice
    event.

    Panic and sustain lift stay charged to their live
...
23. **HOST-INV-023 — a subscription is the host's, bounded, lossy, and invisible to the plan.**
    ...
    **Built by `P05-S009` in its reachable half.** `synth_engine_v2::observe::ObservationSubscriptions` is the
    host's store over one compiled plan's declared taps: a subscription names a `TapSlot` the plan already holds
    and is refused by name for a slot of another plan, an index the plan has no tap for, or a tap already
    subscribed; each admitted subscription owns a preallocated ring of `telemetry_ring_frames` frames of the tapped
    port's layout; the renderer, handed the store for one call like the ingress store, pushes each rendered
    quantum into every ring after the schedule walk and evicts the oldest frames a reader has fallen behind,
    counting them; a read returns the frames, the frames evicted since the last read and how far behind the
    newest quantum the reader stands. It reads a compiled plan and changes nothing — the same plan renders the
    same samples with no subscriber, one, and one that never reads. Owed: subscribing and unsubscribing while
    the stream renders across a thread boundary, decimation, the versioned facade, and Phase 9's verification of
    the live contract under a real host.
...
| A live bounded queue overflows at runtime, or an offered live note-on cannot acquire its producer release hold or an identity from its producer's range | The item is dropped before acceptance and counted against the offered queue; the report names which of slot, hold or identity was exhausted (HOST-INV-009) | Structured diagnostics report |
