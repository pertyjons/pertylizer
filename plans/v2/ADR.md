...
| ADR-0006 | Parameter ramp representation               | Proposed | 5/7                   | [ADR](decisions/ADR-0006-parameter-ramp-representation.md)          | Drafted 2026-09-04 with a linear segment per parameter slot — current value, target, frames remaining; a retarget starts from the current value; sample-positioned controls are never smoothed — as the selected option, coupled with ADR-0007; awaits the user's decision before Phase 5 composes the parameter layers |
| ADR-0007 | Parameter modulation laws                   | Proposed | 5/7                   | [ADR](decisions/ADR-0007-parameter-modulation-laws.md)              | Drafted 2026-09-04 with a closed set of declared laws composed centrally in the parameter slot — normalized/bipolar/semitone/decibel/physical additive, multiplicative gain, thresholded boolean, not-modulatable — ratified from V1's own `set_mod_offset` implementations; awaits the user's decision before Phase 5 composes the parameter layers |
