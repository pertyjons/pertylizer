...
pub fn setup(
    ...
    for x in 0..GRID_SIZE {
        for z in 0..GRID_SIZE {
            ...
            let color = Color::hsl(200.0, 0.7, 0.4);
...
pub fn update(
    ...
    *sim_accum -= steps as f32 * step_interval;
    ...
    // Write back state and update transforms every frame
    // Material colors only update every 3rd frame to reduce GPU re-uploads
    *frame_counter = frame_counter.wrapping_add(1);
    ...
    for (mut cell, mut transform, material_handle) in &mut query {
        ...
        // Material color update (throttled)
        if update_materials {
