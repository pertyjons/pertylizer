...
A real-time 3D visualizer built with Bevy that renders audio-reactive visuals driven by OSC telemetry from the
Pertylizer synthesizer.
...
The visualizer listens on UDP port 9000 for OSC data. When the synth is running, visuals react in real time to audio
levels, FFT spectrum, note events, beat position, and more.
...
The visualizer maps this data to 3D scenes composed of layered visual effects, with automatic theme coloring and camera
movement.

An idle mode reduces update rate when no synth is connected — telemetry resumes automatically on reconnection via a
ping/pong handshake.
