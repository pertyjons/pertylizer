...
- **New MCP tool `analyze_note`** — runs an offline render and returns quantitative metrics: fundamental Hz with anchored search range, pitch-error in cents, peak/RMS/DC, per-window RMS and centroid envelopes, attack/sustain/release spectrum snapshots (top-N peaks), per-window pitch envelope, stereo correlation, 4-band energy split, harmonic content (THD, odd/even ratio, visible-harmonic count), ADSR-like envelope estimate, centroid trend slope, and red-flag booleans (silent, clipping, DC, low output, off-pitch). Lives in `crates/pertylizer/src/audio/analysis.rs` with `synth_dsp::FftProcessor` doing the heavy lifting and a small `MagnitudeWorkspace` so per-window callers reuse the planner. Result types in `synth_mcp::types`.
- **`render_note_to_buffer` extracted from `render_note_preview`.** `audio/preview.rs` now exposes an f32 render path that both `preview_note` (which then calls a new `encode_buffer_as_wav`) and `analyze_note` consume, instead of duplicating the engine setup.
- **Preview honors the patch's octave offset.** `EngineState::octave_offsets` is read by `render_note_to_buffer` and the played note is shifted before NoteOn/NoteOff. Without it, a bass patch with `octave_offset = -2` would preview two octaves higher than it sounds live.
- **New MCP tool `take_screenshot`** — captures the egui viewport and returns PNG bytes. Wired through `ViewportCommand::Screenshot` → `Event::Screenshot` → PNG encode → condvar to the MCP thread. **Currently broken on Wayland**: the compositor throttles `ctx.request_repaint()` from a non-GUI thread when the synth window doesn't have focus, so the GUI loop stays paused and the request times out. Tool returns a clear timeout error explaining the cause; tracked in `docs/TODO.md`.
- **Filter: CV input now scaled to ±48 semitones.** Both the direct cutoff_cv path and the mod-matrix `mod_offset_cutoff` destination multiply by 48, so a full envelope through either route gives a 4-octave sweep. New `env_amt` parameter (default 0.25 ≈ 1 octave at full envelope) lets each filter dial its own range, and a direct cable from an envelope is now just as expressive as routing through the matrix.
- **Mod Matrix: `EnvFollower(0..1)` as a new mod source.** Lets a follower drive a destination through the matrix without forming a graph cycle (a direct CV cable from the follower into something upstream of the follower's own audio input would be rejected by the topo sort). `gather_mod_source_values` in `synth_engine/src/graph.rs` was extended; the per-voice source-values array in `synth_engine/src/voice.rs` grew two slots.
- **Patch param-name fix: `master level` → `master`.** The descriptor parameter on `StereoOutput` is `master`, so every patch that called `param_f("master level", X)` was silently dropping the value and falling back to the default. Renamed across all ~60 built-in patches.
- **Bass patch tweaks** to take advantage of the new filter scaling and matrix EFL routing:
  - `auto_wah_bass`: filter cutoff CV now goes through `EnvelopeFollower → ModMatrix → flt1_cutoff` (a direct cable would form `flt → amp → efl → flt` and be rejected). Faster release, higher sensitivity, `env_amt = 0.5` for a 2-octave wah.
  - `aggressive_bass`: lower sustain (0.6 → 0.35) so the note actually decays rather than swelling on the tube distortion's saturation curve.
  - `sub_bass`: oscillator level 0.5 → 0.3 so the sub clearly dominates; key-tracked filter (cutoff 300 → 500); soft-clip drive 0.15 → 0.4 / mix 0.3 → 0.5 for warmth.
  - `wave_folder_bass`: cutoff 600 → 1200 + key tracking so the wave-folder's added harmonics (the whole point of west-coast synthesis) stay audible.
  - `spacey_bass`: similar polish.
- **Parameter display formatting fixed for descriptor-typed units.** `mcp_bridge::list_modules` was calling `format_param_display(p, None)` first and then re-formatting with the descriptor's unit; the name-based fallback misformats e.g. EFL attack/release stored as `Milliseconds`. Resolved the unit once and format once.
- **MCP-applied patches now reflect their actual engine state in the GUI.** `reconcile_with_session` fetches live module snapshots after detecting added modules and copies parameter values into the GUI's patch_editor, so MCP-applied patches no longer render with descriptor defaults.
- **Code reuse pass** on the analysis module: switched from a hand-rolled `realfft` planner + Hann window to `synth_dsp::FftProcessor` + `synth_dsp::fill_window`; replaced ad-hoc MIDI-to-Hz / cents math with `Hertz::from_midi` and `Hertz::cents_between`; replaced raw stereo-buffer indexing with `StereoSample::iter_frames` + `to_mono`; merged the original `analysis_extras.rs` into `analysis.rs` (deduped Hann/window/FFT helpers); single-pass stereo correlation; `f32::total_cmp` for the peak sort. Added four `From` impls so `analyze_rendered_note` collapses field-by-field copies into `.into()`. Dropped the direct `realfft` dep — pertylizer now reaches it transitively through `synth_dsp`.
