# Parameter → `ParamKind` Audit (Phase 0)

> **Purpose.** The authoritative classification of every engine parameter variant
> into one of the five value-kinds, produced as **Phase 0** of
> [`plans/param-type-system-plan.md`](../plans/param-type-system-plan.md). It is the
> spec the `ScalarParam` impls and the per-enum `kind()` methods must match, and the
> falsification test for the five-kind taxonomy (every variant fits a kind → taxonomy
> confirmed complete).
>
...
> type. Re-run the audit if param enums change. Supersedes the *provisional* counts in
> the plan's §0 (which had no `Reference` bucket and over-counted `Enum`).
