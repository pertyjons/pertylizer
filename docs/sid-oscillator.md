> **Archived design reference.** Moved here from `plans/sid-oscillator-module.md`
> (removed in `b871f057` when shipped plan docs were consolidated) so the `§N`
> section citations in `crates/synth_modules/src/sid_oscillator.rs` and
> `crates/synth_core/src/params/sid_oscillator.rs` keep resolving. Kept verbatim —
> the section numbers (§0–§11) are load-bearing.
